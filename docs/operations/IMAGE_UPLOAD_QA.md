...
```text
[ ] Title を入力し、5MB 以下の .jpg ファイルを選択する
[ ] ファイル選択直後にフォーム内にサムネイルプレビューが表示される
[ ] プレビュー下にファイル名と「X.XMB」形式のサイズが表示される
[ ] 「削除」ボタンが表示されている
[ ] Add をクリックするとボタンが "Saving..." になる
...
### 1-5. プレビューの「削除」ボタンで選択を解除する

```text
[ ] 有効な画像（jpeg/png/webp、5MB 以下）を選択してプレビューを表示させる
[ ] 「削除」ボタンをクリックする
[ ] プレビュー画像・ファイル名・サイズ表示が消える
[ ] ファイル入力がリセットされる（同じファイルを再選択できる）
