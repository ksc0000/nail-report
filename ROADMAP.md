...
- [x] レイアウトモード3種（Sphere/Grid/Spiral）の実装
- [x] スキン4種のクオリティ調整（Snow Globe ガラス球改善含む）
- [x] 詳細表示の中央フォーカス演出（専用スポットライト、スムーズアニメーション）
- [x] 写真アップロード機能（Nano Banana 2 API 連携の土台）
- [x] サンプルネイルデザイン20件の用意
- [ ] **5名以上のターゲットユーザーへのヒアリング**

#### Phase 1 完了定義（Done Definition）
Phase 1 を「完了」とみなす条件：

- [ ] GitHub Pages または Vercel で Web デモを公開し、URL を共有できる
- [ ] iPhone Safari（実機）で 3D 表示・操作が問題なく動くことを確認
- [ ] Sphere / Grid / Spiral の 3 モードを実際に操作し、最も直感的なデフォルトを決定
- [ ] Glass / Snow Globe / Velvet / Showcase の 4 スキンを操作し、本採用スキンを絞り込む
- [ ] ネイルカードタップ時の詳細表示方針を文書化（表示項目・アニメーション仕様）
- [ ] サンプルネイル 20 件のデータ構造を確定し、iOS 実装用スキーマを定義
- [ ] 5 名以上のターゲットユーザー（20〜40 代、ネイル好き女性）に見せ、5 段階評価を回収
- [ ] 「ジュエリー感がある」の平均スコア 4.0 以上を達成
- [ ] React 版または SwiftUI 版へ移植するための仕様書（UX・3D パラメータ・プロンプト）を作成

**成果物**: 公開可能な Web デモ + 体験仕様書（Swift 実装の指針となる）
